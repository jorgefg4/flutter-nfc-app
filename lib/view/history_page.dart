...
class _HistoryPageState extends State<HistoryPage> {
  final GlobalKey<_HistoryPageState> _readPageKey =
  ...
  void _mostrarDialogoGuardarContacto(
      BuildContext context, List<String> contactoInfo) {
    showDialog(
      context: context,
            ...
            TextButton(
              onPressed: () {
                nfcHandler.addContact(
                    contactoInfo[0], contactoInfo[1], contactoInfo[2]);
                        ...
                        children: List.generate(
                          ...
                          (index) {
                            if (index < historial.length) {
                              if (historial[index].startsWith("URL:")) {
                                return Card(
                                  child: ListTile(
                                    ...
                                    subtitle: Text('URL'),
                                    onTap: () {
                                      nfcHandler.launchURL(
                                          historial[index].substring(4));
                                    },
                                  ),
                                );
                              } else if (historial[index]
                                  .startsWith("CONTACTO:")) {
                                String contactoData =
                                    historial[index].substring(9);
                                ...
                                return Card(
                                  child: ListTile(
                                    ...
                                    onTap: () {
                                      _mostrarDialogoGuardarContacto(
                                          context, palabras);
                                    },
                                  ),
                                );
                              ...
                              return SizedBox
                                  .shrink(); // retorna widget vacío y sin espacio
                            }
                          },
                        )
                            .toList()
                            .reversed
                            .toList(), // Invierte el orden de la lista
                      ),
                    ),
            )
          : Center(
