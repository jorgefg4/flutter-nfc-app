...
class NfcHandler {
  final _flutterNfcHcePlugin = FlutterNfcHce(); //instancia plugin
  ...
  void writeUrl(String urlToWrite) async {
    try {
      ...
      //     await _flutterNfcHcePlugin.isSecureNfcEnabled();
      //
      // //isNfcEnabled
  ...
  void writeContact(
      String nameToWrite, String phoneToWrite, String emailToWrite) async {
    try {
      //nfc content
      var content =
          "CONTACTO:" + nameToWrite! + "*" + phoneToWrite! + "*" + emailToWrite;
  ...
  void writeLocation(
      double latitudeToWrite, double longitudeToWrite, String address) async {
    try {
      //nfc content
      var content = "LOCATION:" +
          latitudeToWrite.toString() +
          "*" +
          longitudeToWrite.toString() +
          "*" +
          address;
  ...
  void writeEvent(String titleToWrite, String locationToWrite,
      String initialDateToWrite, String endDateToWrite) async {
    try {
      //nfc content
      var content = "EVENT:" +
          titleToWrite! +
  ...
  void addEvent(
      String title, String location, String initialDate, String endDate) async {
